...
module Slack::Bot::Commands
  ...
  abstract struct Command
    ...
    macro command(name)
        ::Slack::Bot::Commands::Command.register {{name.id.stringify}}, self
        def self.command
          {{name}}
        end
      end
    ...
    macro args(*args)
        ...
        def self.parse(text : String)
          parsed = ::Slack::Bot::Commands::ParsedCommand.new(text)
          index = 0
          {% for arg in args %}
            ...
            %var{arg.var} = ({{arg.type}}).from_slack_bot_value(var)
          {% end %}
...
def Union.from_slack_bot_value(value : String?)
  if value
    {% begin %}
      {% for type in T %}
        {{type}}.from_slack_bot_value(value) ||
      {% end %}
      nil
    {% end %}
  end
...
def Time.from_slack_bot_value(string : String)
...
def Nil.from_slack_bot_value(value)
end

def String.from_slack_bot_value(value : String)
  value
end

def Int32.from_slack_bot_value(value : String)
  value.to_i
end
